...
func New(svc ec2stateservice.Ec2StateSvc, logger log.Logger) Set {
  var powerOnEndpoint endpoint.Endpoint
  {
    powerOnEndpoint = MakePowerOnEndpoint(svc)
    powerOnResponse = LoggingMiddleware(log.With(logger, "method", "poweron"))(powerOnEndpoint)
  }
  var powerOffEndpoint endpoint.Endpoint
  {
    powerOffEndpoint = MakePowerOffEndpoint(svc)
    powerOffResponse = LoggingMiddleware(log.With(logger, "method", "poweroff"))(powerOffEndpoint)
  }
  return Set{
